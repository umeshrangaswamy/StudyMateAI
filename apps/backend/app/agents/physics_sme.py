...
from prompts import PHYSICS_SME_SYSTEM_PROMPT

logger = logging.getLogger(__name__)

class PhysicsSMEAgent:
    """
    SME Agent specializing in Physics curriculum, board exams, 
    and entrance exams (KCET/NEET). Supports doubt solving, lesson explanations,
    summaries, quick notes, formula explanations, and numeric problem guidelines.
    """
    def __init__(self):
        self.vertex_service = VertexAIService()
        logger.info("PhysicsSMEAgent initialized.")
    ...
    async def generate_response(
        ...
        """
        Generates structured Physics pedagogical explanations based on RAG context.
        Adheres strictly to the requested response style constraints.
        """
        logger.info(f"Physics SME generating response for intent={intent}, exam={exam}, response_style={response_style}")
        ...
        if is_insufficient:
            logger.info("RAG context is insufficient or query is out-of-scope for Physics SME.")
        ...
        system_instruction = PHYSICS_SME_SYSTEM_PROMPT.format(
            intent=intent,
