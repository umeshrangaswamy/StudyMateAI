...
# Deploy Cloud SQL Postgres Database Instance
bash infra/gcloud/03-create-cloudsql.sh

# Configure Service Account Runner with minimum permissions
...
# Build container and Deploy backend to Cloud Run (Free Tier constraints)
...
* **Instances bounds**: Configures Cloud Run with `min-instances=0` (scale to zero when idle) and `max-instances=2` (caps token scaling load costs).
* **API permissions**: Grants precise IAM roles (secretAccessor, datastore.user, storage.objectViewer, etc.) to the runner service account, adhering to minimal access policies.
